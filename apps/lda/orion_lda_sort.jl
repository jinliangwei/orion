...
println("topic_assignments.id = ", topic_assignments.id)
...
Orion.set_write_buffer(word_log_gamma_sum_buff, word_log_gamma_sum, apply_buffered_vec)

Orion.@accumulator llh = 0.0

Orion.@dist_array word_topic_vec_table = Orion.fill((Dict{Int32, Int64}(), # topic to index in vec
                                                     Vector{Tuple{Int64, Int32}}()), # (topic_count, topic)
                                                    (vocab_size,))
...
Orion.@parallel_for for word_topic_dict_pair in word_topic_table
    ...
    topic_to_index_map = Dict{Int32, Int64}()
    ...
    for count_index in eachindex(word_topic_vec)
        topic = word_topic_vec[count_index][2]
        topic_to_index_map[topic] = count_index
    end
    word_topic_vec_table[word_id] = (topic_to_index_map, word_topic_vec)
end
...
@time for iteration = 1:num_iterations
    ...
    Orion.@parallel_for for topic_assignment_pair in topic_assignments
        ...
        word_topic_to_index_map = word_topic_vec_pair[1]
        ...
        for c = 1:word_count
            ...
            # use old doc_q_coeff to update the q_terms
            old_topic_index = word_topic_to_index_map[old_topic]
            ...
            if new_topic in keys(word_topic_to_index_map)
                new_topic_index = word_topic_to_index_map[new_topic]
                ...
                word_topic_to_index_map[new_topic] = new_topic_index
                num_nonzero_q_terms += 1
            end
        ...
        doc_topic_table[doc_id] = doc_topic_dict
        word_topic_vec_table[word_id] = (word_topic_to_index_map, word_topic_vec)
        q_coeff[doc_id] = doc_q_coeff
    end

    Orion.@parallel_for for word_topic_pair in word_topic_vec_table
        word_topic_to_index_map = word_topic_pair[2][1]
        word_topic_vec = word_topic_pair[2][2]
        empty!(word_topic_to_index_map)
        sort!(word_topic_vec, by = x -> x[1], rev = true)
        num_nonzeros = 0
        for (topic_count, topic) in word_topic_vec
            ...
            num_nonzeros += 1
            word_topic_to_index_map[topic] = num_nonzeros
        end
    ...
    if iteration % 1 == 0 ||
        ...
        Orion.@parallel_for for word_topic_pair in word_topic_vec_table
            ...
            for topic = 1:num_topics
                topic_index = get(word_topic_to_index_map, topic, 0)
                if topic_index == 0
                    count = 0
                else
                    count = word_topic_vec[topic_index][1]
                end
