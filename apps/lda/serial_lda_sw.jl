...
function parse_line(index::Int64, line::AbstractString)::Vector{Tuple{Tuple{Int64, Int64},
    ...
    for token in tokens[2:end]
        ...
        word_count_dict[word_id] = parse(Int64, word_count[2])
    end
...
function init(docs, topic_summary, doc_topic_assignments, word_topic_table, doc_topic_table)
...
init(docs, topic_summary, doc_topic_assignments, word_topic_table, doc_topic_table)
...
function sample_docs(docs, topic_summary, doc_topic_assignments,
                     word_topic_table, doc_topic_table,
                     alpha_beta, beta_sum)
    for doc_id in eachindex(docs)
        ...
        for token_count in doc
            ...
            for c = 1:count
                ...
                for (topic, topic_count) in word_topic_dict
                    q_term = q_coeff[topic] * topic_count
                ...
                s_sum -= (alpha_beta) / denom
                s_sum += alpha_beta / (denom + 1)
                if get(doc_topic_dict, new_topic, 0) != 0
                ...
                if get(word_topic_dict, new_topic, 0) != 0
...
function evaluate(docs, topic_summary, doc_topic_assignments,
                  word_topic_table, doc_topic_table,
                  alpha_beta, beta_sum)
    llh = 0.0
    for k = 1:num_topics
