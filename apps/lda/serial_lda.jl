...
function sample_one_word(topic_assignment_key,
                         old_topic,
                         doc_topic_table,
                         ...
                         old_doc_id,
                         q_sum,
                         num_nonzero_q_terms)::Tuple{Int32, Float32, Int64, Int64, Float32, UInt64}
    ...
    return new_topic, s_sum, old_word_id, old_doc_id, q_sum, num_nonzero_q_terms
end
...
function sample_all_words(topic_assignments,
    ...
    num_nonzero_q_terms = UInt64(0)
    for idx in eachindex(topic_assignment_keys)
        topic_assignment_key = topic_assignment_keys[idx]
        topic = topic_assignment_topics[idx]
        results = sample_one_word(topic_assignment_key,
                                  topic,
                                  doc_topic_table,
                                  word_topic_vec_table,
                                  topic_summary,
        ...
        new_topic = results[1]
        s_sum = results[2]
        old_word_id = results[3]
        old_doc_id = results[4]
        q_sum = results[5]
        num_nonzero_q_terms = results[6]
        topic_assignment_topics[idx] = new_topic
    end
    return s_sum
end
...
srand(1)
@time initialize(topic_assignments, word_topic_dict_table, doc_topic_table,
           topic_summary)
