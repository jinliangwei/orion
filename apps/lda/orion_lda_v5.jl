...
Orion.@share function create_word_topic_vec(word_topic_dict::Dict{Int32, UInt64}
...
q_term_val = Vector{Float32}(num_topics)
q_term_topic = Vector{Int32}(num_topics)
...
@time for iteration = 1:num_iterations
    ...
    Orion.@parallel_for repeated for topic_assignment_pair in topic_assignments
        ...
        if new_topic_index != -1
            ...
            q_sum -= q_term_val[new_topic_index][1]
        else
...
loss_fobj = open("lda.loss.nytimes.v5", "w")
